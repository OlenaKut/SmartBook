...
1.  **User (Loan) Card**
    Each user in the system is represented by a Card ID and a Name.
    This simple system allows the library to track which user has borrowed which books.
    There was created a class `User` with the following properties:

- `CardId` (string) - Id of the User.
- `Name` (string) - Name of the User.

Method `RegisterUser` in `Library.cs` adds a new user to the system if their Card ID doesn't already exist.
Method `FindUserByCardId` in `Library.cs` returns a user based on their Card ID.
Method `LoanBook` in `Library.cs` asks user's ID and then loans a book to a specific user.
We can view all the books borrowed by the user by selecting 8 from the menu.

2. **Export a Report of Loaned Books**
The library system can automatically generate a .txt file containing a list of all currently loaned books.
The method `ExportLoanedBooksReport` in `Library.cs` generates and writes the report for all books where IsLoaned == true.
All loaned books are saved to loaned_books.txt file. This file updates every time a book is loaned, returned or removed. 

3. **Export logs**
The library system can automatically generate a .txt file containing all logs
The method `LogAction` in `Library.cs` generates a new log when methods `RegisterUser`, `AddBook`, `RemoveBook`, `LoanBook`, and `MarkAsAvailable` are called.